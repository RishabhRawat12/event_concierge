...
class TacticalAuthenticator:
    """Enterprise-grade Firebase JWT validation."""
    @staticmethod
    def verify(credentials: HTTPAuthorizationCredentials = Security(security)) -> dict:
        try:
            return fb_manager.verify_token(credentials.credentials)
        except ValueError as e:
            logger.warning(f"UNAUTHORIZED ACCESS: {e}")
            raise HTTPException(
                status_code=403, 
                detail="Forbidden: Invalid or expired authentication token."
            )

def get_client_ip(request: Request) -> str:
    """Determine the real client IP (Respecting Trusted Proxy status)."""
...
async def rate_limit_ai(request: Request) -> None:
    """AI Routes Quota: 10 requests per minute."""
    ip = get_client_ip(request)
    if await cache.is_rate_limited(f"ratelimit:ai:{ip}", capacity=10, window=60):
        logger.warning(f"AI Quota Exhaustion for {ip}.")
        raise HTTPException(
            status_code=429, 
            detail="Too Many Requests: AI orchestration quota exhausted."
        )

async def rate_limit_state(request: Request) -> None:
    """State Routes Quota: 100 requests per minute."""
    ip = get_client_ip(request)
    if await cache.is_rate_limited(f"ratelimit:state:{ip}", capacity=100, window=60):
        logger.warning(f"State Quota Exhaustion for {ip}.")
        raise HTTPException(
            status_code=429, 
            detail="Too Many Requests: State orchestration quota exhausted."
        )

# Route Architectures with Universal Security Dependencies
# Route Architectures with Universal Security Dependencies
router = APIRouter(tags=["Attendee Flow"], dependencies=[Depends(rate_limit_ai)])
staff_router = APIRouter(tags=["Staff Tactical Flow"], dependencies=[Depends(rate_limit_ai)])
