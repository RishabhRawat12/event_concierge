...
class Event(BaseModel):
    ...
    event_name: Annotated[str, StringConstraints(min_length=1, max_length=100)] = Field(..., description="Canonical name of the event")
    start_time: str = Field(..., pattern=r"^\d{1,2}:\d{2} [AP]M$", description="Formatted event start time")
    end_time: str = Field(..., pattern=r"^\d{1,2}:\d{2} [AP]M$", description="Formatted event end time")
    walking_directions: Annotated[str, StringConstraints(max_length=500)] = Field(..., description="Pathfinding narration between venues")
...
class ItineraryResponse(BaseModel):
    ...
    current_weather: Optional[Annotated[str, StringConstraints(max_length=50)]] = Field(default=None, description="Metereological context for navigation")
...
class StaffActionRequest(BaseModel):
    ...
    zone_id: Annotated[str, StringConstraints(min_length=1, max_length=50)] = Field(..., description="Target zone identifier (e.g. 'Zone B')")
    alert_type: Annotated[str, StringConstraints(min_length=1, max_length=50)] = Field(..., description="Metric driving the tactical deployment (e.g. 'Crowd Density')")

class StaffActionResponse(BaseModel):
    """Tactical protocol generated by the orchestration engine."""
    model_config = ConfigDict(populate_by_name=True)

    protocol: Annotated[str, StringConstraints(max_length=1000)] = Field(..., description="Narrated personnel deployment instructions")
    simulated: bool = Field(default=False, description="Indicator for deterministic fallback results")
